...
def mel_filterbank(nfft, nfiltbank, fs):
    ...
    for i in range(1, nfiltbank + 1):
        for k in range(int(nfft / 2 + 1)):
            if k < fbins[i - 1]:
                fbank[k, i - 1] = 0
            elif fbins[i - 1] <= k < fbins[i]:
                fbank[k, i - 1] = (k - fbins[i - 1]) / (fbins[i] - fbins[i - 1])
            elif fbins[i] <= k <= fbins[i + 1]:
...
def mfcc(s, fs, nfiltbank):
    ...
    # compute periodogram
    naff = 512
    periodogram = np.empty((nFrames, naff / 2 + 1))
    for i in range(nFrames):
        x = segment[:, i] * hamming(nSamples)
        spectrum = fftshift(fft(x, naff))
        periodogram[i, :] = abs(spectrum[naff / 2 - 1:]) / nSamples

    # calculating mfccs
    fbank = mel_filterbank(naff, nfiltbank, fs)
